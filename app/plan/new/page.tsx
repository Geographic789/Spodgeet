...
type Distance = { id: string; label: string; distance_km: number; elevation_gain_m: number | null; start_time: string | null };
...
function NewPlanForm() {
  ...
  const [selectedRaceId, setSelectedRaceId] = useState(params.get("raceId") || "");
  const [selectedDistanceId, setSelectedDistanceId] = useState(params.get("distanceId") || "");
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    const res = await fetch("/api/plans", {
      ...
      body: JSON.stringify({ distanceId: selectedDistanceId, userName, planName, goalTimeStr: goalTime }),
    });
    setCreating(false);
    if (res.ok) {
        ...
        <form onSubmit={handleCreate} className="card space-y-5 p-7">
            ...
            <select className="field-input" value={selectedDistanceId} required
              ...
              {distances.map(d => (
                <option key={d.id} value={d.id}>
                  {d.label} ({d.distance_km.toFixed(1)} km{d.elevation_gain_m ? ` · +${Math.round(d.elevation_gain_m)}m` : ""})
                </option>
              ))}
            </select>
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-ink/40">
              Target finish time · pace calculated automatically using terrain and fatigue
          ...
          <button className="btn-primary w-full py-3 text-base" disabled={creating}>
            {creating ? "Building plan…" : "Create plan →"}
          </button>
        </form>
      </main>
    </div>
...
export default function NewPlanPage() {
  return <Suspense fallback={<div className="p-8 text-ink/50">Loading…</div>}><NewPlanForm /></Suspense>;
}
