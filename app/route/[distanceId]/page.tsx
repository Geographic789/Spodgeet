...
export default function RouteViewerPage() {
  ...
  const hoverGradient = useMemo(() => {
    ...
    return chartPoints.reduce((prev, cur) =>
  ...
  // Pre-selection link for plan creation
  const createPlanUrl = `/plan/new${distance ? `?distanceId=${distance.id}&raceId=${distance.race_id}` : ""}`;
        ...
        {distance && (
          <>
            <div className="flex items-start justify-between">
              ...
              {/* Pre-selects this race + distance on the plan creation form */}
              <Link href={createPlanUrl} className="btn-primary text-xs">
                Create pacing plan →
              </Link>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 min-h-[24px]">
              {hoverCumKm !== null ? (
                <>
                  <span className="font-mono font-semibold text-ink text-sm">{hoverCumKm.toFixed(1)} km</span>
                  {hoverGradient !== null && (
