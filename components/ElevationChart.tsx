...
export default function ElevationChart({
  ...
  const handleTouchMove = useCallback((e: React.TouchEvent) => {
    e.preventDefault();
    const c = pixelToCumKm(e.touches[0].clientX);
    if (c !== null) onHover(c);
    ...
    >
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          {/* Full opacity bars colored by gradient */}
          <Bar dataKey="ele" isAnimationActive={false} maxBarSize={6}>
            {data.map((entry, i) => (
              <Cell key={i} fill={gradientColor(entry.gradient)} fillOpacity={1} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
